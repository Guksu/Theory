...
- API 후킹은 주로 Win32 API가 호출 될 때 중간에서 제어권을 얻어낸다.
- static방식은 작업 대상이 파일이고 dynamic방식은 작업 대상이 프로세스이다.
- static
...
- Hooking은 일반적으로 IAT, Code, EAT 세 군데를 공략한다.
- IAT

```
API 주소를 후킹 함수 주소로 변경하는 방법
구현은 쉽지만 IAT에는 없지만 프로그램에서 사용되는 API는 후킹이 불가능하다.
```

- Code

```
프로세스 메모리에 로딩되어 있는 시스템 DLL에서 API의 실제 주소를 찾아서 코드를 직접 수정하는 방법
```

- EAT

```
DLL의 Export Address Table에 기록되어 있는 API의 시작 주소를 후킹 함수 주소로 변경하는 방법
```

## **WebHook**

- Server에서 지정된 클라이언트 주소로 API를 역으로 보내준다

```
메일이 왔는지 확인하는 경우
일반적인 REST API의 경우 클라이언트측에서 지속적으로 확인 API를 호출해야하지만
WebHook의 경우엔 서버에서 한번만 호출하여 효율적이다.
```

- 서버에서 특정 작업 수행시 이를 HTTP Post로 알려준다.
- 주로 한 시스템에서 발생하는 이벤트가 다른 시스템에 전달되어 처리되어야 할 때 사용한다.
- 단, 클라이언트 측에서 서비스가 중단될 경우 WebHook으로 전달되는 데이터는 유실된다.
